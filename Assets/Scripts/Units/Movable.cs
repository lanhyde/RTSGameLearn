...
{
    /// <summary>
    /// This module allows unit to move. Do not add it to the buildings.
    /// </summary>
    public class Movable : Module
    {
        ...
        void Start()
        {
            ...
            {
                Debug.LogWarning("[Movable module] Unit " + name + " has disabled Has Move module toggle, but Movable module still added to the prefab. Fix this");
            }
        ...
        void Update()
        {
            if(destination != transform.position)
            {
                ...
                // TODO: check if the target is to the right or left and turn the wheels in that direction.
                MoveToPosition(lastMovePosition);
                // reach destination
                if((transform.position - destination).sqrMagnitude <= sqrDistanceFineToStop)
                {
                    Stop();
                }
            }
