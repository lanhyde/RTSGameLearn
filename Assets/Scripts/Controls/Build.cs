    ...
    public class Build : MonoBehaviour
    {
        ...
        void HandleBuilding(Vector3 buildingSize)
        {
            ...
            if(Physics.Raycast(ray, out hit, 1000, terrainLayer))
            {
                ...
                if(Input.GetMouseButtonDown(0) && GameController.instance.MainStorage.allowBuildingsRotation)
                {
                    if(startedRotation)
                    {
                        drawer.transform.rotation = Quaternion.LookRotation(hit.point - drawer.transform.position);
                    }
                    else if(Vector3.Distance(drawer.transform.position, hit.point) > 1.5f)
                    {
                ...
                else
                {
                    ...
                    {
                        finalHitPoint = new Vector3((int)hit.point.x, (int)hit.point.y, (int)hit.point.z);
                    }
