...
namespace PromiseCode.RTS.Controls
{
    public static class Ordering
    {
        ...
        public static void SendOrderToUnits(List<Unit> units, Order order, bool isAdditive)
        {
            ...
            if (usedFormation == Storing.Storage.UnitsFormation.Default)
            {
                wayPoints = UnitsFormations.GetWaypointsForUnitsGroup(movePosition, units);
            }
            else if(usedFormation  == Storing.Storage.UnitsFormation.SquarePredict)
            {
            ...
            for(int i = 0; i < units.Count; ++i)
            {
                ...
                if(order is FollowOrder && units[i].data.moveType == Storing.UnitData.MoveType.Flying)
                {
