...
namespace PromiseCode.RTS.UI.Minimap
{
    public class MinimapCameraIcon : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        {
            var offset = eventData.position - (Vector2) minimap.IconsPanel.position +
                         Vector2.one * minimap.MapImageSize / 2f;       // Add Half of map size because draw zone have centered pivot
        ...
        bool IsPositionIconOutOfBounds(Vector2 position) => position.x < 0 || position.y < 0 ||
                                                            position.x > minimap.MapImageSize ||
                                                            position.y > minimap.MapImageSize;
